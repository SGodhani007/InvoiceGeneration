...
<html>
  ...
  <body>
    <div class='container pt-5'>
      <!-- <h1 class='text-center text-primary'>Anthem Infotech Billing Department</h1><hr> -->
      <nav id="navbar">
        <div class="flex flex-row">
         <span><h1>Anthem Infotech Pvt. Ltd.</h1><a href="admin.php"><button id="btnadmin" style="height:50px;width:140px;background-color:#13e7b9;border-radius:20px;color:white;">Admin</button></a><h3>Last Invoice No. </h3></span>
        </div>
      </nav>
      <br>
      <?php
       if(isset($miidate['invoiceno'])){
        $miidate = $midate['invoiceno'];
    }
          $select_query = "SELECT INVOICE_NO from invoice";
          $result_query = mysqli_query($con, $select_query);

          while($row_data = mysqli_fetch_assoc($result_query)){
            $mino = $row_data['INVOICE_NO'];
      ...
      <?php
        if(isset($_POST["submit"])){
          ...
          $sql="insert into invoice (INVOICE_NO,INVOICE_DATE,CNAME,CADDRESS,C_CITY,GRAND_TOTAL) values ('{$invoice_no}','{$invoice_date}','{$cname}','{$caddress}','{$ccity}','{$grand_total}') ";
          if($con->query($sql)){
            $sid=$con->insert_id;
        ...
        <div class='row'>
          <div class='col-md-4'>
            ...
            <div class='form-group'>
              ...
              <input type='text' name='invoice_no' id="invoiceno" required class='form-control'>
            </div>
            <div class='form-group'>
          ...
          <div class='col-md-8'>
            ...
            <div class='form-group'>
              <label>Name</label>
              <input type='text' name='cname' id="cname1" required class='form-control'>
            </div>
            <div class='form-group'>
              <label>Address</label>
